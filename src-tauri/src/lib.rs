...
use vtl_core::engine::{self as engine_mod, Engine as _, ModelType, DeviceType};
...
struct AppState {
    ...
    recorder: Recorder,
    player: Player,
    engine: Option<Box<dyn engine_mod::Engine>>,
}
...
fn load_engine(config: &AppConfig) -> Option<Box<dyn engine_mod::Engine>> {
    ...
    let (model_path, tokens_path) = build_model_paths(&models_dir_str, &config.model.active_model_id, &model_type)?;
...
fn build_model_paths(
    ...
    match model_type {
        ...
        _ => {
            println!("engine: unsupported model type '{}' in path builder", model_type);
            None
        }
    }
}

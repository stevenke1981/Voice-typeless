...
**最後更新**：2026-06-11
**狀態**：Rust Core + Tauri v2 + Svelte 5 持續開發中
...
- 全新專案名稱與品牌
- 技術棧全面現代化（Tauri v2 + 模組化 Rust Core）
...
| **Core**                | 錄音、識別、熱鍵、粘貼、AI 後處理   | Rust 1.77+ + Tauri 後端              | 單獨 crate，可獨立測試           |
...
| **Tester**              | 單元 + E2E + 跨平台測試             | Cargo test + Playwright + Tauri test | 覆蓋率 > 92%                     |
...
- **後端核心**：Rust 1.77+（模組化 `vtl-core` library crate）
...
## 5. 全新專案結構（從零設計）
Voice-typeless/
├── core-rs/                # 獨立 Rust library crate（可重用）
│   ├── src/engine/         # SenseVoice / Whisper.cpp 引擎抽象
│   ├── src/audio/          # cpal 錄音、裝置、VAD、提示音
│   ├── src/hotkey.rs       # 快捷鍵資料模型與解析
│   ├── src/paste.rs        # 粘貼與剪貼板保護
│   └── src/processor.rs    # 後處理與語氣詞過濾
├── frontend/               # Svelte 5 + Vite + TailwindCSS
├── src-tauri/              # Tauri v2 桌面橋接與命令
├── plugins/                # 未來插件目錄
├── models/                 # 模型工具與測試素材
├── docs/                   # 規格、架構、API、知識圖譜、經驗紀錄
